...
This directory contains only Venworks-authored Scaleform timeline and
ActionScript patch definitions, probe fixtures, build manifests, hashes, and
...
The build exports Bethesda ActionScript only into ignored temporary storage,
verifies the authored patch anchors, imports only the patched `HUDMenu` class,
and confirms that the reopened output contains the required loader and
diagnostic values. Full exported classes are never repository source.
...
Files under `shared/fixtures` are developer test inputs. The malformed fixture
is intentionally not well-formed XML.
