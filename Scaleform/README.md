...
`createPackages.ps1` resolves each selected variant's auxiliary profile and requires the staged `Interface/venworkscui.swf` hash to match the production manifest before it deletes or writes any archive. This deliberately blocks the local marker probe and stale auxiliary output from every Windows, Xbox, and PlayStation package path.
