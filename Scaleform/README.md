...
- Apache Flex SDK 4.16.1 with the pinned Flash Player 11.1 `playerglobal.swc`
...
Run `../Tools/Setup-ScaleformEnvironment.ps1 -AcceptAdobeLicense` from this directory, or `./Tools/Setup-ScaleformEnvironment.ps1 -AcceptAdobeLicense` from the repository root, to install or repair the complete ignored toolchain. The setup pins immutable artifact URLs, lengths, and SHA-256 values for Temurin, JPEXS, Apache Flex, the Adobe Flex SDK archive, and the extracted Player 11.1 SWC. Pass `-ArtifactCachePath "C:\path\to\private\ScaleformArtifacts"` to reuse a private local cache across worktrees. The setup trusts the compiled-in hashes rather than any adjacent checksum sidecar, downloads an absent artifact from its pinned official URL, and stores a verified download in the selected cache. Installing the Adobe payload requires explicit license acceptance, and none of the downloaded archives or extracted SDK files may be committed.
...
`buildVariant.ps1` independently compiles each selected host manifest profile from clean GFX and SWF inputs, compiles shared HUD-message GFX/SWF pairs only for profiles that declare them, and compiles each selected standalone CUI source profile once. The deployment mapping stages each native GFX output at its `.gfx` path and each independently compiled CWS output at its `.swf` path. The five player-facing variants each receive nine movies from `Scaleform/variants/<KEY>/build.psd1`: four base HUD paths, four HUD-message movies, and `venworkscui.swf`. The four themed profiles share the production layout, 11-name SWF component inventory, SVG assets, external palettes, and standalone CUI hash. Minimalist uses its own faction-free layout, references nine applicable names from the same 11-component registry, supplies literal Starfield colors through the `minimalist` definition variant, and has a profile-specific standalone CUI hash with no external component XML, SVG, palette, or DDS files. PS5 Debug selects four unique host manifests that patch the existing lifecycle pane, production loader, and auxiliary-phase observer into Bethesda's existing single ABC. It stages those four host movies plus a one-class diagnostic `venworkscui.swf` that preserves the successful dynamically resolved `PlayerData` subscription, then loads one isolated `Interface/VenworksCUI/layout.xml`, parses one bounded `diagnosticText` value, and keeps the sanitized `sName` and current XML phase on separate persistent rows. It still has no HUD-message movies, production components, SVG, palettes, assets, provider contexts, or full CUI runtime. Omitting `-VariantKeys` selects all six release variants. Pass one key, such as `-VariantKeys PS5DBG`, or an array, such as `-VariantKeys @("TA", "PS5DBG")`, to select a subset. The singular `-VariantKey` name remains a compatibility alias. Use `-Committed` to regenerate tracked staging directories without Vortex Junctions.

`compileScaleform.ps1` is the lower-level HUD host compiler. Each invocation preserves the build input container contract: `.gfx` inputs must carry a `GFX` signature and generate GFX outputs, while `.swf` inputs must carry a `CWS` signature and generate CWS outputs. Its manifest mode applies one or more ordered bounded host patches, and every mode patches Bethesda's existing HUD ABC without adding another `DoABC` tag. The player-facing manifests install the auxiliary loader; the PS5 Debug manifests compose the existing lifecycle/error pane, that same shared loader, and an observer that reports its phases through the existing pane. The variant builder, not the compiler, applies the explicit deployment mapping that preserves each output's native extension and container. `compileScaleformAuxiliary.ps1` compiles either the complete CUI runtime or the PS5 Debug one-class bridge into a deterministic CWS auxiliary movie using Apache Flex and JPEXS normalization/reopen validation. Production runtime profiles additionally use compile-only Bethesda externs; the diagnostic bridge instead resolves `Shared.AS3.Data.BSUIDataManager` at runtime, preserves only its single `PlayerData` subscription, and permits the isolated URL load and E4X parse needed for the diagnostic text probe. The auxiliary manifest fixes the movie at 1920 by 1080, 30 fps, and one frame, matching Bethesda's HUD stage metadata; the `_lrg` host remains 1920 by 1080 rather than using an ultrawide stage. Neither compiler mirrors variant configuration or runs Archive2. Deploy each production CUI profile's generated `layout.xml` with the matching `venworkscui.swf`; the shipped layout resolves its 11 supplied names synchronously from definitions compiled into that movie. Legacy `<include>` imports remain supported for separately authored custom fragments, use relative filenames resolved only inside `Interface/VenworksCUI/components`, and reject nested paths and traversal. The production `quest-tracker` SWF component joins directly beneath the faction and radar panels, binds the existing `HudCompassData` tracked-objective text, and remains independent of scanner and aiming state while leaving its text field blank when the objective is empty. The objective resolver is independent of floating-marker visibility. Vanilla quest markers, icons, arrows, distance labels, and temporary quest notifications remain under Bethesda ownership. `HUDMessagesMenu` is patched at its `MonocleMenu_Opened` handler so opening the on-foot scanner keeps only the always-up objective list hidden; ship-scanner behavior is unchanged. The HUDMenu-owned Watch/environment cluster is configured with `visibleWhen="never"`, which applies real display visibility without positional hiding. Bethesda's original Monocle movies retain scanner input, labels, and survey placement. No HUD-wide frame callback or cross-movie display lookup is used.

The auxiliary compiler records two independent SHA-256 contracts: the final movie bytes and the compiled class inventory. Runtime bridges embed the current transformed-source and class-inventory fingerprints into `VenworksCUIEntrypoint`; diagnostic bridges embed their derived class-inventory fingerprint into the declared document class. Verification reads those fingerprints from the staged movie and compares them with the current manifest, externs, generated Scaleform stub, entrypoint, profiled ActionScript, or exact diagnostic inventory as applicable. A source or definition change therefore cannot pass by leaving an older auxiliary SWF and its old expected movie hash in place.

`createPackages.ps1` resolves each selected variant's complete deployment profile and requires that profile's exact Interface inventory, production hashes, and declared native GFX/CWS signatures before it deletes or writes any archive. This enforces nine movies for every player-facing profile and four one-ABC host movies, one one-ABC diagnostic auxiliary movie, and one isolated diagnostic XML file for PS5 Debug, while blocking the local marker probe, stale output, wrong containers, extra configuration, and incomplete mappings from every selected platform archive path.
