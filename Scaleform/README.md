...
`createPackages.ps1` resolves each selected variant's complete deployment profile and requires the exact nine-movie inventory, production hashes, declared signatures, and byte-identical normal/large host alias pairs before it deletes or writes any archive. This blocks the local marker probe, stale output, native-GFX host aliases, and incomplete mappings from every Windows, Xbox, and PlayStation package path.
