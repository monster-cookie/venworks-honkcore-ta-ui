...
This Creation starts with `crimson-fleet.xml`, but every variant includes all five stock palettes.
...
4. Find the root `<venworksCUI>` element and change only its `palette` attribute, for example `palette="starfield.xml"`.
