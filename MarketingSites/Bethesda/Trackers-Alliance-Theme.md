...
A from-scratch, XML-customizable Starfield HUD with the Trackers Alliance palette, tactical data, a contact radar, and five included color palettes.

# Description Markdown

# Overview

Venworks Customizable HUD overhauls Starfield's interface with a new HUD written from the ground up in Scaleform and ActionScript 3. This Creation starts with the Trackers Alliance palette and crest. Every gauge, contact, status bar, and information panel has a defined gameplay or interface purpose; this is not a static mockup placed over the game.
...
This Creation starts with `trackers-alliance.xml`, but every variant includes all five stock palettes.
...
4. Find the root `<venworksCUI>` element and change only its `palette` attribute, for example `palette="venworks.xml"`.
