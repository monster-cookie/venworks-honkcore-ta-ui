# Venworks Customizable HUD tooling policy

Policy-Version: 1

This repository policy applies only to this checkout of `venworks-honkcore-ta-ui`, as identified by its existing repository context and reviewed origin. Resolve it with the adopted shared policies using AgentKit's tooling-and-credentials procedure. Repository entries replace shared entries with the same ID in full. Unspecified services keep their existing authorized workflow.
...
| Roles | all |
| Requirement | preferred |
| Tool | PowerShell 7 via `pwsh -NoProfile`, using the repository's existing `Tools/` scripts and documented parameters. Relevant entry points include `Tools/checkRepo.ps1`, `Tools/checkRepoV2.ps1`, `Tools/buildVariant.ps1`, `Tools/buildVariantV2.ps1`, and their existing packaging scripts. Inspect the selected script, its imports, configuration, and side effects before execution. |
| Identity | none |
| Target | This checkout of `venworks-honkcore-ta-ui` and the specific local inputs and output paths authorized by the task. |
...
| Identity | none |
| Target | Architecture documentation, technical documentation, and authorized pull-request descriptions for `venworks-honkcore-ta-ui`. |
