...
foreach ($variant in $variants) {
  ...
  elseif ($hasDiagnosticXmlPayload) {
    ...
    $diagnosticXhtmlPattern = '\A<\?xml version="1\.0" encoding="utf-8"\?>[ \t\r\n]*<html>[ \t\r\n]*<head>[ \t\r\n]*<title>(?<Title>[^<&\r\n\t]{1,80})</title>[ \t\r\n]*</head>[ \t\r\n]*<body>[ \t\r\n]*<section>[ \t\r\n]*<h1>(?<Heading>[^<&\r\n\t]{1,80})</h1>[ \t\r\n]*<p>(?<Paragraph>[^<&\r\n\t]{1,80})</p>[ \t\r\n]*</section>[ \t\r\n]*</body>[ \t\r\n]*</html>[ \t\r\n]*\z'
    $diagnosticXhtmlMatch = [regex]::Match(
      $diagnosticXmlSourceText,
      $diagnosticXhtmlPattern,
      [System.Text.RegularExpressions.RegexOptions]::CultureInvariant
    )
    if (!$diagnosticXhtmlMatch.Success) {
      throw "$($variant.VariantName) diagnostic XHTML must match the exact declaration and html/head/title/body/section/h1/p text grammar accepted by the bounded runtime parser."
    }
    try {
    ...
    foreach ($diagnosticHtmlValue in $diagnosticHtmlValues) {
      if ($diagnosticEntrypointSource.Contains($diagnosticHtmlValue)) {
        throw "$($variant.VariantName) diagnostic ActionScript must not embed layout-derived XHTML text."
      }
    }
    ...
    foreach ($requiredDiagnosticSourceToken in @(
      ...
      'isLayoutWhitespace',
      'this.xhtmlPane.text =',
      'this.xhtmlPane.setTextFormat'
    )) {
      if (!$diagnosticEntrypointSource.Contains($requiredDiagnosticSourceToken)) {
        throw "$($variant.VariantName) diagnostic ActionScript is missing required bounded XHTML token '$requiredDiagnosticSourceToken'."
      }
    }
  }
  ...
  elseif ($movieProfile.AuxiliaryContract -ceq 'diagnostic-bridge') {
    ...
    foreach ($requiredDiagnosticToken in @(
      ...
      'PS5DBG-08 XHTML TEXT LOAD NEXT FRAME',
      'PS5DBG-09 XHTML LOAD RETURNED',
      'PS5DBG-10 XHTML TEXT RECEIVED',
      'PS5DBG-11 XHTML PARSE NEXT FRAME',
      'PS5DBG-12 BASIC XHTML RENDER',
      'PS5DBG-OK PLAYERDATA',
      'PS5DBG-ERR PLAYERDATA',
      'PS5DBG-OK XHTML',
      'PS5DBG-ERR XHTML REQUEST',
      'PS5DBG-ERR XHTML IO',
      'PS5DBG-ERR XHTML SECURITY',
      'PS5DBG-ERR XHTML TEXT',
      'PS5DBG-ERR XHTML SIZE',
      'PS5DBG-ERR XHTML PARSE',
      'PS5DBG-ERR XHTML RENDER',
      'PLAYERDATA:',
      'XHTML:',
      'URLRequest',
      'URLLoader',
    ...
    foreach ($diagnosticHtmlValue in $diagnosticHtmlValues) {
      if ([string]::IsNullOrWhiteSpace($diagnosticHtmlValue) -or $auxiliaryInspection.Text.Contains($diagnosticHtmlValue)) {
        throw "$($variant.VariantName) diagnostic venworkscui.swf must not embed layout-derived XHTML text."
      }
    }
