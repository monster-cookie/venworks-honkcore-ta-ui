...
The diagnostic parse is limited to the production runtime's established `new XML`, root-name, and direct-child E4X operations. The diagnostic source and reopened movie must not declare `XMLList` values or traverse with `elements()`. The static XML verifier still requires exactly one direct text-only `diagnosticText` element under the unnamespaced `venworksCUI` root; runtime acceptance reads that value through the production-compatible direct-child lookup.
