...
For Vortex deployment, first run `.\Tools\setupRepo.ps1 -VariantKeys PS5DBG` to replace the tracked staging directory with the configured local junction, then run the same build, package, and verification commands without `-Committed`. Ordinary mode continues to reject missing, non-junction, or incorrectly targeted staging paths; committed build, package, and verification mode operates on the tracked directory without converting it into a junction.
...
The exact staged payload is the unique ESM plus `hudmenu.gfx`, `hudmenu.swf`, `hudmenu_lrg.gfx`, and `hudmenu_lrg.swf`. The generic archive command creates only Windows `Main.ba2` and PlayStation `Main_PS.ba2`, both General archives with `compression=None`. Windows exists for the required PC diagnostic gate, while the generic release-packaging command emits only the Bethesda PS5 ZIP. It deliberately creates no Bethesda PC, Xbox, Nexus, or fully loose release package.

`verifyVariant.ps1 -VariantKeys PS5DBG` rejects any extra file, archive, movie, ABC, CUI token, compressed BA2 entry, mismatched hash, or noncanonical ESM bytes. `verifyCommittedRelease.ps1` and CI verify the diagnostic profile through the same six-variant registry and profile resolver. A successful PC result proves only that the diagnostic host movies load locally; VWKSHUD-37 remains in review until a PS5 end user reports whether the pane reaches `PS5DBG-OK HUD LOADED` or displays a `PS5DBG-ERR` phase.
