...
The provider-free v2.0.6 test produced no change in the reported PS5 crash, so Minimalist again retains its required live providers and provider-driven CUI events. Every Bethesda-path movie is still built independently from the matching clean container: native `.gfx` inputs produce validated native GFX intermediates, and ZLIB-compressed `.swf` inputs produce validated CWS outputs. For the v2.0.10 PS5 compatibility probe, only the normal and large HUD deployment mapping aliases the CWS output under both runtime names; HUD-message paths retain the native GFX/CWS split. The separately authored auxiliary runtime is emitted as compressed CWS version 12. Native rectangle and ellipse fills remain in the fitted holographic backings.
...
Every platform Main archive packages the staged nine-movie inventory directly: byte-identical CWS aliases at the `.gfx` and `.swf` paths for the normal and large HUD, native GFX plus independently compiled CWS for the two HUD-message pairs, and the profile-selected `venworkscui.swf`. Windows, Xbox, and PlayStation use the same source inventory and archive contract; there is no platform-only movie rewrite. Minimalist's platform packages contain its ESM and matching Main BA2 with no texture archive, while its fully loose Nexus package contains the nine movies plus the reduced external XML tree. Generated XML and SVG payloads use UTF-8 without a byte-order mark and canonical LF line endings so committed BA2 contents remain byte-identical to clean checkouts on Windows and Linux. The complete release matrix contains 25 ZIPs. Every normal Nexus package leaves only `layout.xml` loose so the compiled HUD movies remain protected by the BA2. Users who need to edit component fragments, palettes, or SVG assets must use a fully loose package or provide a separate loose override. All five variants remain eligible for both Nexus package shapes, but the v2.0.10 probe sets the global and every variant-specific Nexus upload switch to `false`; only Minimalist is intended for manual Creations publication during PS5 acceptance. Do not install a normal and fully loose package together.
