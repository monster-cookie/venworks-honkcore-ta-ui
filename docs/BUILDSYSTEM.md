...
The normal v1 entrypoints remain the release pipeline. The opt-in `buildVariantV2.ps1`, `compileScaleformAuxiliaryV2.ps1`, `verifyVariantV2.ps1`, `createPackagesV2.ps1`, `createReleasePackagesV2.ps1`, `verifyCommittedReleaseV2.ps1`, and `checkRepoV2.ps1` form a parallel experimental pipeline for the v2.0.18 PS5Debug probe. The v2 chain retains the v1 contracts for all five player-facing variants and changes only the PS5Debug auxiliary contract. It continues to share the stable host compiler, override compiler, configuration, movie, profile, and reference-cache helpers. Do not treat a v2 result as a normal v1 release-pipeline result or switch CI/release routing until the PS5 experiment is accepted.
...
`verifyVariantV2.ps1 -VariantKeys PS5DBG` rejects any extra file, archive, movie, ABC, unexpected runtime/provider token, compressed BA2 entry, mismatched hash, or noncanonical ESM bytes. It requires the diagnostic auxiliary's exact one-class inventory, bridge API, Starfield-font panes, allowlisted dynamic `PlayerData` and text-loading phases, expected movie/class hashes, and an exact UTF-8 no-BOM LF copy of the profile-selected layout. The layout must match the exact declaration and `html/head/title/body/section/h1/p` grammar accepted by the bounded parser, with text-only title, heading, and paragraph values that are not embedded in the ActionScript or compiled SWF. `verifyCommittedReleaseV2.ps1` verifies the experimental profile through the same six-variant registry and profile resolver without changing the normal v1 verifier or CI entrypoint. The v2.0.15 PS5 result closes the PlayerData gate. The v2.0.17 archive-only PC result passed, but the PS5 test produced no visible diagnostic and hard-rebooted the console. A successful v2.0.18 PC result proves only that this bounded XHTML translation can initialize locally; it still requires its own PS5 result.
