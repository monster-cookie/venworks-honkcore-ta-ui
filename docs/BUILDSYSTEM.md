...
`verifyVariant.ps1 -VariantKeys PS5DBG` rejects any extra file, archive, movie, ABC, unexpected runtime/provider token, compressed BA2 entry, mismatched hash, or noncanonical ESM bytes. It requires the diagnostic auxiliary's exact one-class inventory, bridge API, Starfield-font panes, allowlisted dynamic `PlayerData` and XML calls and phases, expected movie/class hashes, and an exact UTF-8 no-BOM LF copy of the profile-selected XML. For v2.0.18 that XML must be an unnamespaced, attribute-free `html` document containing exactly `head/title` followed by `body/section/h1/p`, with text-only title, heading, and paragraph values that are not embedded in the ActionScript or compiled SWF. `verifyCommittedRelease.ps1` and CI verify the diagnostic profile through the same six-variant registry and profile resolver. The v2.0.15 PS5 result closes the PlayerData gate, and v2.0.17 has passed its PC PlayerData-plus-XML gate while its PS5 result remains pending. A successful v2.0.18 PC result proves only that this bounded basic-HTML translation can initialize locally; it still requires its own PS5 result.

The v2.0.18 diagnostic parse remains limited to `new XML`, the root-name check, and the direct `head.title`, `body.section.h1`, and `body.section.p` E4X paths. The diagnostic source and reopened movie must not declare `XMLList` values, traverse XML children or descendants, use `htmlText` or `StyleSheet`, expose `ExternalInterface`, or navigate to another URL. It renders one additional fixed native `TextField`, assigns its ordinary `text` value, and applies separate heading and paragraph `TextFormat` ranges. This is a bounded basic-HTML experiment, not a browser, CSS engine, JavaScript host, namespace-aware XHTML parser, or production CUI replacement.
...
After any bootstrap, diagnostic host patch, entrypoint, CUI class, provider profile, or extern change, run the complete six-variant `Tools/buildVariant.ps1` command, `Tools/createPackages.ps1`, and `Tools/verifyCommittedRelease.ps1`. A successful build must reopen and validate every generated movie and regenerate each selected platform Main archive from its profile-declared Interface inventory. Before a player-facing release, force one missing-auxiliary load to confirm the diagnostic is readable, then test at least Minimalist and one themed variant from an archive-only PC install containing only the selected ESM and Main BA2 with no loose Interface shadow. The v2.0.15 PlayerData PS5 gate has passed, and v2.0.17 has passed its PC XML gate while awaiting PS5 acceptance. Before the v2.0.18 basic-HTML probe is submitted, test its Windows Main BA2 on PC and confirm `PS5DBG-03 AUX LOAD STARTED`, `PS5DBG-04 AUX INITIALIZED`, `PS5DBG-OK AUX COMPLETE`, a persistent `PLAYERDATA:` row containing `PS5DBG-OK PLAYERDATA | <name>`, a simultaneous `XML:` row, and the separate rendered heading-and-paragraph panel. `PS5DBG-08 XML LOAD NEXT FRAME` identifies a load call that has not returned, `PS5DBG-09 XML LOAD RETURNED` proves the call returned while awaiting an event, `PS5DBG-10 XML RECEIVED` and `PS5DBG-11 XML PARSE NEXT FRAME` prove later boundaries, `PS5DBG-12 BASIC HTML RENDER` identifies native text-field creation, and `PS5DBG-OK HTML | html/head/title/body/section/h1/p` proves the bounded translation completed. A contained `PS5DBG-ERR XML` or `PS5DBG-ERR HTML` phase records the failing boundary. Confirm teardown without a late callback fault. Rollback restores the v2.0.17 diagnostic auxiliary and archives; the four diagnostic hosts and every player-facing movie and archive remain unchanged.
