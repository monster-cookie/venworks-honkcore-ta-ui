...
Use the PS5 Debug build profile when the purpose is to advance the console isolation test one boundary at a time. v2.0.15 loaded successfully on PS5 and displayed the correct player name, while v2.0.17 has passed its PC PlayerData-plus-XML gate and awaits its PS5 result. The v2.0.18 successor preserves those probes and adds only one bounded, well-formed basic-HTML translation into a native text field before any production Venworks runtime or configuration is introduced:
...
The PS5 Debug manifests select the same lower-level compiler and existing shared loader used by the player-facing host movies. Ordered patches add the lifecycle pane, shared loader, and a small observer that reports the loader's existing phases; they modify only the existing `HUDMenu` class in Bethesda's existing ABC and do not add another `DoABC` tag or custom host document class. The output contains the four normal/large Bethesda HUD movie paths, one diagnostic `venworkscui.swf`, one `VenworksCUI/layout.xml`, and the uniquely named PS5 Debug plugin. The child movie contains only `VenworksCUIDiagnosticEntrypoint`, its embedded class-inventory fingerprint, its four untyped bridge methods, the two-row Starfield-font status pane, and the fixed native basic-HTML result pane. It first identifies `venworkscui.swf loaded`, then the next frame resolves `Shared.AS3.Data.BSUIDataManager` by name, primes and subscribes only `PlayerData`, and displays sanitized, bounded `sName` text or a contained error. A valid callback renders the pre-call XML boundary, invokes the diagnostic XML loader on the following frame, records whether that call returns, defers its E4X parse to another visible boundary, and requires the exact unnamespaced `html/head/title/body/section/h1/p` tree. The title, heading, and paragraph must be text-only, trimmed, single-line values of at most 80 characters. The renderer assigns ordinary `TextField.text` and applies separate `TextFormat` ranges; it never invokes Scaleform's disabled HTML engine. Do not add CSS, JavaScript, `htmlText`, namespaces, traversal, links, remote resources, HUD-message movies, production layout semantics, components, SVG, palettes, assets, production provider contexts, other providers, the production CUI runtime, Xbox archives, or Nexus package shapes to this diagnostic. The generic archive builder retains a Windows Main BA2 for the PC gate and a PS5 Main BA2 for Creations; the release-package matrix exposes only the Bethesda PS5 ZIP.

Interpret the top-center pane by the last visible phase: `PS5DBG-01 CONSTRUCTED` proves the document-class constructor ran, `PS5DBG-02 ADDED TO STAGE` proves the HUD instance joined the display list, `PS5DBG-03 AUX LOAD STARTED` proves the shared loader issued its request, `PS5DBG-04 AUX INITIALIZED` proves the child bridge initialized, and `PS5DBG-OK AUX COMPLETE` proves the child root completed and attached. The v2.0.15 PS5 result establishes the PlayerData baseline, and the v2.0.17 PC result establishes the local XML baseline while its PS5 result remains pending. The child status pane retains a `PLAYERDATA:` row and an `XML:` row simultaneously. `PS5DBG-05 PLAYERDATA NEXT FRAME`, `PS5DBG-06 PLAYERDATA REQUEST`, `PS5DBG-07 PLAYERDATA WAITING`, `PS5DBG-OK PLAYERDATA | <name>`, and `PS5DBG-ERR PLAYERDATA` retain their established meanings. After a valid name arrives, `PS5DBG-08 XML LOAD NEXT FRAME` is rendered before the isolated load call; if it remains visible, that call did not return. `PS5DBG-09 XML LOAD RETURNED` proves the call returned while no terminal event had fired, `PS5DBG-10 XML RECEIVED` proves text arrived, `PS5DBG-11 XML PARSE NEXT FRAME` identifies the E4X boundary, and `PS5DBG-12 BASIC HTML RENDER` identifies native text-field creation. `PS5DBG-OK HTML | html/head/title/body/section/h1/p` plus the separate visible `VENWORKS HUD` heading and paragraph proves the bounded translation completed. `PS5DBG-ERR XML REQUEST`, `IO`, `SECURITY`, `PARSE`, or `VALUE` identifies the contained loading stage; `PS5DBG-ERR HTML VALUE` or `RENDER` identifies the new boundary. Top-pane `PS5DBG-ERR` still records a contained host or loader failure and its phase. Once auxiliary completion is recorded, the top pane suppresses older non-error host lifecycle updates; an error may still replace completion. Once an error is recorded, the top pane suppresses every later non-error update so it cannot report false success. Absence of either pane does not prove the movie was absent; it means the failure occurred before the relevant diagnostic could render and must be correlated with deployment hashes and the unique ESM/BA2 inventory.

The v2.0.18 diagnostic E4X parse uses only `new XML`, the root-name check, and direct `head.title`, `body.section.h1`, and `body.section.p` access. It adds no `XMLList` declarations, XML traversal, namespace handling, mixed inline markup, CSS, JavaScript, links, or browser behavior. `PS5DBG-OK HTML | html/head/title/body/section/h1/p` proves those values reached the native renderer; it does not establish support for any other HTML element or attribute.
...
- every Main BA2 has the exact staged Interface inventory, including only the one diagnostic XML file for PS5 Debug, stores every entry without BA2 compression, and remains byte-identical to its staged source;
