...
Use the PS5 Debug build profile when the purpose is to advance the console isolation test one boundary at a time: v2.0.15 is now reported loading successfully on PS5 and displaying the correct player name, so this successor preserves that exact `PlayerData` probe and adds only one isolated XML request, E4X parse, and text display before any production Venworks runtime or configuration is introduced:
