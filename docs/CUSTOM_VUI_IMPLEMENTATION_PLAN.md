...
4. scanner, quest, and quick-access surfaces;
...
status effects, or scanner presentation.

Stealth/detection presentation, hit/kill/damage indicators, weapon reticles,
and crosshairs are also deliberately outside the configurable surface. These
engine-controlled visuals remain exclusively vanilla-owned. Crosshair switching
may be technically safe, but the vanilla presentation is adequate and replacing
it provides no meaningful benefit. Proven crosshair, first/third-person,
sneaking, and combat fields remain available as read-only condition inputs for
the threat model and approved layout choreography without transferring visual
ownership.
